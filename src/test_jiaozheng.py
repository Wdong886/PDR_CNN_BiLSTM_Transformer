...
# 计算路径位置
def compute_positions_all(timestamps, directions, step_length, real_positions=None, correction_interval=200):
    x, y = 0, 0
    positions = [(x, y)]
...
if __name__ == "__main__":
    ...
    for timestamp, pos in zip(mag_data["Time (s)"], positions):
        print(f"时间戳: {timestamp:.9f} 秒 -> 位置: x = {pos[0]:.6f}, y = {pos[1]:.6f}")
    ...
    # 将估算位置中的每200个时间戳校正为真实位置
    corrected_positions = compute_positions_all(mag_data["Time (s)"], directions, step_length,
                                                real_positions=real_positions)
    # 去除最后一条估算记录，避免长度不匹配
    corrected_positions = corrected_positions[:-1]
    # 将估算位置和真实位置保存到新的 Excel 文件
    modified_data = pd.read_excel(DATA_PATH + 'data.xlsx')
    ...
    newpath=DATA_PATH + 'modified_data.xlsx'
    # 保存新的文件
    ...
    # 读取保存的估算数据文件
    modified_data = pd.read_excel(newpath)

    # 读取估算位置和真实位置
    estimated_positions = modified_data[['Estimated X', 'Estimated Y']].values
    real_trajectory = pd.read_excel(DATA_PATH + 'data.xlsx', usecols=[17, 18])  # 第18、19列为真实轨迹
    real_positions = real_trajectory.values  # 获取真实轨迹的 x 和 y 坐标
    ...
    # 保存到新的Excel文件
    #modified_data.to_excel(DATA_PATH + 'error/zhicheng_leg2_data_with_errors.xlsx', index=False)
