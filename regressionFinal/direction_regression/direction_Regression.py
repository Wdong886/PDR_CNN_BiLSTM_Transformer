...
from keras.losses import Huber  # 替代 tensorflow_addons 的 PinballLoss
...
# 保存真实值与预测值对到 txt 文件中
results = np.column_stack((y_val_orig, y_pred_calibrated))
np.savetxt('direction_regression_results.txt', results, fmt='%.4f', header='True\tPredicted', comments='')
...
print(f"验证集均方误差 (MSE): {mse:.4f}, 平均绝对误差 (MAE): {mae:.4f}, R²: {r2:.4f}")



# 绘制回归线
x_vals = np.linspace(min(y_val_orig), max(y_val_orig), 100)
y_vals_reg = slope * x_vals + intercept
plt.plot(x_vals, y_vals_reg, color='green', linestyle='-', linewidth=2, label='Regression Line')
...
# ----- 绘制误差分布直方图 -----
errors = y_pred_calibrated - y_val_orig
