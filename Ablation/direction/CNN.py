...
from keras.layers import Input, Conv1D, BatchNormalization, MaxPooling1D, Dropout, GlobalAveragePooling1D, Dense
...
model = build_model_cnn((sequence_length, num_features))
...
# ========= 保存模型与可视化 =========
model.save('model_scheme1_cnn.h5')
for interval, calibrator in calibrators.items():
    dump(calibrator, f'calibrator_scheme1_{interval}.joblib')
dump(scaler, 'scaler_scheme1.joblib')
dump(selector, 'selector_scheme1.joblib')
dump(y_scaler, 'yscaler_scheme1.joblib')
...
plt.legend()
plt.title('Scheme1: CNN - True vs. Predicted')
plt.show()
...
rmse = np.sqrt(mse)
print(f"CNN--MSE: {mse:.4f}, MAE: {mae:.4f}, R²: {r2:.4f}, RMSE: {rmse:.4f}")
