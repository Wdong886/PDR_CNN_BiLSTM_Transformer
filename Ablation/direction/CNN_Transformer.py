...
from tensorflow.keras.layers import Input, Conv1D, BatchNormalization, MaxPooling1D, Dropout, MultiHeadAttention, Add, LayerNormalization, Dense, GlobalAveragePooling1D
...
# ===================== 模型构建（修复维度） =====================
def build_model_cnn_transformer(input_shape):
    ...
    ffn = Dense(128)(ffn)  # 128 与 CNN 输出一致
    x = Add()([x, ffn])
    x = LayerNormalization()(x)
...
model = build_model_cnn_transformer(input_shape)
...
print(f"CNN_Transformer: 验证集 MSE: {mse:.4f}, MAE: {mae:.4f}, R²: {r2:.4f}")
